...
username = 'username'
password = 'password'
region = 'ap' # ap = asia pacific, some other regions are eu, na, etc. try out different region codes and confirm if it matches your store
...
def get_bundles(skins_data, bundles_data, weapons_data):
    ...
    for bundle in bundles:
        ...
        print(f'Bundle: {info["description"]} - {bundle_price} VP')
        print('Weapons:')
        for item in bundle['Items']:
            ...
            try:
                ...
                print(f'\t{item_name} - {item_price} VP')
            except:
                continue
        print('-----------------------------------------------')
...
def get_data(user_id, headers, region):
    ...
    bundles_data = requests.get(f'https://valorant-api.com/v1/bundles')
    bundles_data = bundles_data.json()['data']

    weapons_data = requests.get(f'https://valorant-api.com/v1/weapons/skinlevels')
...
def get_skins(skins_data, weapons_data, offers_data):
    ...
    for uuid in skins:
        ...
        skin_price = prices[uuid]
        print(f'\t{skin_name} - {skin_price} VP')
...
def get_night_market(skins_data, weapons_data):
    ...
    for item in night_market:
        ...
        skin_price = [item['DiscountCosts'][i] for i in item['DiscountCosts']][0]
        skin = [i for i in weapons_data if i['uuid'] == uuid][0]
        skin_name = skin['displayName']
        print(f'\t{skin_name} - {skin_price} VP')
...
def main():
    ...
    get_skins(skins_data, weapons_data, offers_data)
    get_night_market(skins_data, weapons_data)
